...
```powershell
smart-search search "今天 OpenAI Responses API 有什么新变化" --capabilities docs_search,web_search --format json
...
Agent 生成的普通搜索会在首次 `search` 调用传入完整 Caller Capability Declaration：按 Catalog 顺序使用 `docs_search,web_search` 这样的 CSV，空集合严格使用 `none`，无需先调用 `route`。人类直接使用 CLI 时可省略 `--capabilities` 以保留 hybrid routing；`research` 保持独立证据流且不接收该选项。
...
```powershell
smart-search search "今天有什么值得关注的 AI 新闻？" --capabilities web_search --validation balanced --extra-sources 2 --format json
```
...
只有可规范化的 HTTP(S) 候选才会从 Vertical Discovery 进入 `extra_sources`。AnySearch 搜索与批量结果的 description 最多 300 字；搜索、批量、域发现和自动 Vertical Discovery 都不返回重复的 `content`、`raw_content` 或 `raw_result`，只有提取操作保留一份 `content`。Agent 需要完整网页时，应对已发现 URL 调用 `smart-search fetch URL`。无 URL 的结构化响应以紧凑 provider 结果保留在 `vertical_discovery` 中，绝不伪装成 source、citation 或 evidence。provider 失败会通过 `vertical_discovery` 和 `provider_attempts` 保留 operation、上游 tool 与错误类别，但不会改变成功主结果。无域响应不会声明或验收具体域。本地重点回归意图包括 academic、gaming 攻略和 travel 行程；普通“游戏”用法或比喻性的“旅行”不会命中。
...
| `vertical_discovery` | AnySearch 无域调用的紧凑 provider 结果；无 URL 内容只以最多 300 字描述保留在这里 |
...
`@jfmoe/smart-search@0.7.1` 是个人发行线。Node 是唯一承诺的 JavaScript
