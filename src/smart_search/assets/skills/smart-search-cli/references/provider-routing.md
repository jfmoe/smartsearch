...
- Automatic Domain Search is not implemented. Explicit AnySearch Acceptance Surface operations may try anonymously; only `ANYSEARCH_API_KEY` marks the provider configured and enables automatic Vertical Discovery. Batch Discovery and AnySearch Extraction are never automatic.
