...
- Route diagnostics accept the same complete, authoritative `--capabilities` declaration as ordinary search. Smart Search does not supplement a declaration from query rules, known URLs, strict validation, embeddings, or the classifier. Choose either caller declaration or `--router-mode`; the CLI rejects combining them because each controls the routing decision.
