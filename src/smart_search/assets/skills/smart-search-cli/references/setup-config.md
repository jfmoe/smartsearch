...
- Context7 uses `CONTEXT7_MCP_API_URL`, defaulting to `https://mcp.context7.com/mcp`, with `CONTEXT7_API_KEY` and `CONTEXT7_TIMEOUT_SECONDS` unchanged. Do not configure or suggest `CONTEXT7_BASE_URL`: it is a retired REST key. If only that old endpoint is present, `doctor` reports the migration and Context7 commands fail closed until `CONTEXT7_MCP_API_URL` is set (or the old key is removed to use the default).
