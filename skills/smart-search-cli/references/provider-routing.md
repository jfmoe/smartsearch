...
- A supplied `--capabilities` declaration is the authoritative complete set; only calls without one run rules, embeddings, or the classifier to select capabilities.
...
- Jina Reader is `web_fetch` only, not a general search provider. `JINA_API_KEY` or a non-empty `JINA_API_KEYS` pool is required before Jina satisfies the standard minimum profile; anonymous `r.jina.ai` is explicit/experimental fetch behavior.
- Provider Credential Pool (allowlisted): Exa, Tavily, Jina, Firecrawl, Context7, and AnySearch accept optional `*_API_KEYS` JSON arrays for multi-credential round-robin and same-call rate-limit rotation. Non-empty KEYS replaces KEY. xAI, OpenAI-compatible, Zhipu, Zhipu MCP, and intent-router credentials stay single-key.
...
- The versioned Verified Domain Manifest is the only support authority. Its verified set is initially empty; `academic.search`, `security.vuln`, `finance.fundamental`, and `code.doc` remain discovered/unverified until every recorded gate is met. Live discovery and domain-less search never promote or infer a Verified Vertical Domain. Runtime status reports configured state, automatic Vertical Discovery, experimental state, operation-live state, verified domains, and candidate assessments separately.
- Automatic Domain Search is not implemented. Explicit AnySearch Acceptance Surface operations may try anonymously; a non-empty AnySearch credential pool (`ANYSEARCH_API_KEY` or `ANYSEARCH_API_KEYS`) marks the provider configured and enables automatic Vertical Discovery. Batch Discovery and AnySearch Extraction are never automatic.
- Default `search` automatic Vertical Discovery requires successful main search, `balanced`/`strict` validation, routed vertical intent, an AnySearch-allowing provider filter, and `ANYSEARCH_API_KEY`. `research` reuses the domain-less call when its balanced intent route selects `vertical_search` and configured AnySearch is available; it exposes neither a provider-filter flag nor a main-search gate. The key is both configured state and the switch for both automatic paths. `--extra-sources 0` controls only Tavily/Firecrawl in `search`. Never add domain, sub-domain, or Sub-domain Parameters: upstream selects its internal data source. Promote only normalized HTTP(S) candidates to `extra_sources`; preserve URL-less structured output under `vertical_discovery` without treating it as source/evidence. Failures must retain operation, tool, and error category without failing a successful main result. Local regression rules cover academic, gaming-guide, and travel-itinerary intent without mirroring the upstream catalog.
...
- `JINA_API_KEY` or non-empty `JINA_API_KEYS` is required before Jina satisfies `SMART_SEARCH_MINIMUM_PROFILE=standard`.
...
- `JINA_RESPOND_WITH=readerlm-v2` requires `JINA_API_KEY` or `JINA_API_KEYS` and should report a configuration error without a network request when no credential is present.
...
- `ANYSEARCH_API_KEY` / `ANYSEARCH_API_KEYS` is optional. If configured, requests include `Authorization: Bearer ...`; if missing, anonymous requests are allowed on explicit Acceptance Surface commands.
...
- Static AnySearch capability status is offline. It reports `configured`, `automatic_vertical_discovery`, `experimental`, manifest-backed `verified_domains`, and each operation as `not_run`. Live smoke requires `ANYSEARCH_API_KEY` or `ANYSEARCH_API_KEYS` in the process environment rather than an undeclared saved key; endpoint and timeout retain the normal environment → saved config → default priority. It reports each Provider Acceptance Operation and selected explicit domain as `passed`, `failed`, or `not_run` with stable `operation`, `tool`, and `error_type` fields. `ANYSEARCH_LIVE_ACCEPTANCE` selects candidate domains; the default is `academic.search` and `all` selects every manifest candidate.
