...
Treat a command as complete only after the runner reports a terminal exit status. If it returns a session or background handle, retain the full result; the next tool call must poll that same handle, and no other command, diagnosis, or output-file read may run until it exits. Use a unique `--output` path for every attempt, then verify the saved query and timeout match that attempt before trusting the file. Empty stdout or a missing file before terminal exit is not a failure.
...
```powershell
smart-search search "query" --capabilities none --extra-sources 5 --timeout 300 --format json --output result.json
smart-search search "latest React API changes" --capabilities docs_search,web_search --stream --format json
smart-search diagnose openai-compatible --format markdown
smart-search search "query" --capabilities none --platform "Reuters" --model "model-id" --extra-sources 3 --timeout 300 --format json
smart-search search "nba战报" --capabilities web_search --format content
smart-search search "query" --capabilities none --validation strict --fallback auto --providers auto --format json
...
Agent timeout handling contract: `smart-search search ... --capabilities CSV_OR_NONE --timeout 300 --extra-sources 1 --format json --output PATH` is the retry shape; reuse the complete declaration from the initial call. It is not a shell-level `timeout` wrapper. `SMART_SEARCH_RETRY_*` settings are not the contract for this path. After repeated timeout failures, switch to source-first fallback with `exa-search`, `exa-search --include-domains`, and fetched evidence. Final answers assembled through that fallback should explicitly label the evidence mode, for example `source_mode: "fallback"` or equivalent prose.
